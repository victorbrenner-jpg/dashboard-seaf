...
_PAGAMENTOS_COLETIVOS = (("BOLSA ATLETA", "BOLSA ATLETA"), ("BOLSA ESCOLA", "BOLSA ESCOLA"))
...
def _tipo_item(valor): return "RETENÇÃO" if "RETEN" in _normalizar(valor) else "ITEM"
...
def _consolidar_pagamentos_coletivos(pdf, base):
    ...
    objetos = base["Objeto_Conferencia"].fillna("").astype(str).map(_normalizar) if "Objeto_Conferencia" in base.columns else pd.Series("", index=base.index, dtype=str)
    itens = base["Tipo_Item_Conferencia"].eq("ITEM")
    numeros = base["Número"].fillna("").astype(str).str.strip()
    for marcador_pdf, marcador_objeto in _PAGAMENTOS_COLETIVOS:
        linhas_pdf = pdf[pdf["Credor_Chave"].fillna("").astype(str).str.contains(re.escape(marcador_pdf), regex=True, na=False)]
        if linhas_pdf.empty: continue
        for _, linha_pdf in linhas_pdf.iterrows():
            gd_pdf, valor_pdf = str(linha_pdf["GD"]).strip(), float(linha_pdf["Valor_PDF"])
            credor_pdf, chave_pdf = str(linha_pdf["Credor"]).strip(), str(linha_pdf["Credor_Chave"]).strip()
            mesmo_gd = base["GD_Conferencia"].astype(str).eq(gd_pdf)
            alvo_objeto = objetos.str.contains(re.escape(marcador_objeto), regex=True, na=False) & itens & mesmo_gd
            if alvo_objeto.any():
                base.loc[alvo_objeto, ["Credor_Exibicao", "Credor_Chave"]] = [credor_pdf, chave_pdf]
                continue
            elegiveis = base[itens & mesmo_gd & numeros.ne("") & ~numeros.isin(["-", "NAN", "NONE"])].copy()
            if elegiveis.empty: continue
            grupos_ob = elegiveis.groupby("Número", dropna=False).agg(Valor_OB=("Valor_Conferencia", "sum"), Registros_OB=("Valor_Conferencia", "size"), Credores_OB=("Credor_Chave", "nunique")).reset_index()
            candidatos = grupos_ob[grupos_ob["Registros_OB"].ge(2) & grupos_ob["Credores_OB"].ge(2) & (grupos_ob["Valor_OB"] - valor_pdf).abs().le(.005)]
            if len(candidatos) != 1: continue
            numero_ob = str(candidatos.iloc[0]["Número"]).strip()
            alvo_ob = itens & mesmo_gd & numeros.eq(numero_ob)
            base.loc[alvo_ob, ["Credor_Exibicao", "Credor_Chave"]] = [credor_pdf, chave_pdf]
    return base
...
def carregar_base_ob():
    try:
        requisicao = urllib.request.Request(LINK_BASE_OB_CONFERENCIA, headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"})
        with urllib.request.urlopen(requisicao, timeout=30) as resposta:
            base = pd.read_csv(io.BytesIO(resposta.read()), dtype=str)
    except Exception: return pd.DataFrame()
    base = base.loc[:, ~base.columns.duplicated()].copy(); base.columns = [str(c).strip() for c in base.columns]
    coluna_objeto = next((c for c in ("Objeto da Despesa", "Objeto Despesa", "Objeto de Despesa", "Objeto") if c in base.columns), None)
    for coluna in ("Número", "Data Emissão", "Valor", "Fonte", "Nome do Credor", "Credor", "Tipo Item", "GRUPO"):
        if coluna not in base: base[coluna] = ""
    base["Objeto_Conferencia"] = base[coluna_objeto].fillna("").astype(str) if coluna_objeto is not None else ""
    ...
    # ITEM pode vir duplicado na exportação e continua protegido pela chave histórica.
    # RETENÇÃO não pode ser deduplicada por valor/credor: existem lançamentos legítimos
    # iguais em OBs diferentes e todos precisam entrar no total do SIAFIM.
    chave = ["Número", "Data Emissão", "Valor", "Fonte", "Nome do Credor", "Tipo Item"]
    itens = base[base["Tipo_Item_Conferencia"].eq("ITEM")].drop_duplicates(subset=chave, keep="first")
    retencoes = base[base["Tipo_Item_Conferencia"].eq("RETENÇÃO")]
    return pd.concat([itens, retencoes], ignore_index=True)


def _linhas_pdf(conteudo):
    if pdfplumber is None: raise RuntimeError("A biblioteca pdfplumber não está instalada. Atualize as dependências da aplicação.")
    ...
    with pdfplumber.open(io.BytesIO(conteudo)) as pdf:
        for pagina in pdf.pages:
            ...
            for palavra in sorted(palavras, key=lambda p: (float(p["top"]), float(p["x0"]))):
                ...
                if alvo is None: alvo = {"top": float(palavra["top"]), "itens": []}; grupos_palavras.append(alvo)
                alvo["itens"].append(palavra); alvo["top"] = sum(float(x["top"]) for x in alvo["itens"]) / len(alvo["itens"])
            cabecalho = None
            for grupo in grupos_palavras:
                itens_g = grupo["itens"]; texto_cab = _normalizar(" ".join(str(x["text"]) for x in itens_g))
                if "NOME DO CREDOR" in texto_cab and "GD" in texto_cab and "VALOR" in texto_cab:
                    gd = next((x for x in itens_g if str(x["text"]).upper() == "GD"), None); valor = [x for x in itens_g if str(x["text"]).upper() in {"VALOR", "TOTAL"}]
                    if gd and valor: cabecalho = {"fim": max(float(x["bottom"]) for x in itens_g), "gd_x": (float(gd["x0"])+float(gd["x1"]))/2, "limite": min(float(pagina.width)*.74, max(float(x["x1"]) for x in valor)+max(25,float(pagina.width)*.035))}
                    break
            if cabecalho:
                grupos_chars=[]
                for ordem,char in enumerate(pagina.chars or []):
                    ...
                    if alvo is None: alvo={"top":float(char["top"]),"itens":[]}; grupos_chars.append(alvo)
                    item=dict(char); item["ordem"]=ordem; alvo["itens"].append(item); alvo["top"]=sum(float(x["top"]) for x in alvo["itens"])/len(alvo["itens"])
                for grupo in grupos_chars:
                    if grupo["top"]<=cabecalho["fim"]+1: continue
                    chars=[x for x in grupo["itens"] if float(x["x0"])<cabecalho["limite"]]
                    candidatos=[x for x in chars if str(x["text"]) in {"3","4"} and abs(((float(x["x0"])+float(x["x1"]))/2)-cabecalho["gd_x"])<=max(9,float(pagina.width)*.012)]
                    if not candidatos: continue
                    gd_char=max(candidatos,key=lambda x:x["ordem"]); antes=[x for x in chars if x["ordem"]<gd_char["ordem"]]; depois=[x for x in chars if x["ordem"]>gd_char["ordem"]]
                    credor="".join(str(x["text"]) for x in antes).strip(); valor=re.search(r"R\$\s*([0-9.]+,[0-9]{2})","".join(str(x["text"]) for x in depois)); chave=_normalizar(credor)
                    if valor and chave and re.search(r"[A-Z]",chave) and not chave.startswith("TOTAL"): estruturais.append({"Credor":credor,"Credor_Chave":chave,"GD":str(gd_char["text"]),"Valor_PDF":float(_valor([valor.group(1)]).iloc[0])})
            texto=pagina.extract_text() or ""; em_tabela,credor_pendente=True,""
            for linha in texto.splitlines():
                limpa=linha.strip(); normal=_normalizar(limpa); totais=padrao_total.findall(limpa.upper())
                if totais: total_declarado=max(total_declarado or 0,float(_valor(totais).iloc[0]))
                if "NOME DO CREDOR" in normal and "GD" in normal and "VALOR" in normal: em_tabela,credor_pendente=True,""; continue
                achado_fonte=re.search(r"(?<!\d)(\d{3})\s+R\$\s*[0-9.]+,[0-9]{2}",limpa)
                if achado_fonte: fontes.append(achado_fonte.group(1))
                if not em_tabela: continue
                if normal.startswith("TOTAL GERAL"): credor_pendente=""; continue
                achado=padrao.match(limpa)
                if not achado:
                    achado=padrao_sobreposto.match(limpa)
                    if achado: credor=(achado.group("ini")+achado.group("fim")).strip(); gd,valor=achado.group("gd"),achado.group("valor")
                    else:
                        quebrada=re.match(r"^([34])\s+R\$\s*([0-9.]+,[0-9]{2})",limpa)
                        if quebrada and credor_pendente: credor,gd,valor=credor_pendente,quebrada.group(1),quebrada.group(2)
                        else:
                            if limpa and "R$" not in limpa and not normal.startswith(("TOTAL","UG ","GD ","FONTE ")): credor_pendente=limpa
                            continue
                else: credor,gd,valor=achado.group("credor"),achado.group("gd"),achado.group("valor")
                chave=_normalizar(credor)
                if chave and re.search(r"[A-Z]",chave) and not chave.startswith("TOTAL"): registros.append({"Credor":credor.strip(),"Credor_Chave":chave,"GD":gd,"Valor_PDF":float(_valor([valor]).iloc[0])})
                credor_pendente=""
    ...
    if not tabela_estrutural.empty:
        ...
        if total_declarado is None or abs(float(tabela_estrutural["Valor_PDF"].sum())-total_declarado)<=.01: return tabela_estrutural,sorted(set(fontes))
    tabela=pd.DataFrame(registros)
    if tabela.empty: raise ValueError("Não foi possível localizar a tabela principal de credores no PDF.")
    tabela=tabela.groupby(["Credor_Chave","GD"],as_index=False).agg(Credor=("Credor","first"),Valor_PDF=("Valor_PDF","sum"))
    if total_declarado is not None and abs(float(tabela["Valor_PDF"].sum())-total_declarado)>.01: raise ValueError(f"A leitura do PDF ficou incompleta: total extraído ({_brl(tabela['Valor_PDF'].sum())}) não confere com o total declarado ({_brl(total_declarado)}).")
...
def comparar(conteudo,data,classificacao):
    ...
    for tipo in ("ITEM","RETENÇÃO"):
        if tipo not in tipos: tipos[tipo]=0.0
    resultado=pdf.merge(agrupada,on=["Credor_Chave","GD"],how="outer").merge(tipos[["Credor_Chave","GD","ITEM","RETENÇÃO"]],on=["Credor_Chave","GD"],how="left"); resultado["Credor"]=resultado["Credor"].fillna(resultado["Credor_Base"])
    for coluna in ("Valor_PDF","Valor_Base","ITEM","RETENÇÃO"): resultado[coluna]=resultado[coluna].fillna(0.0)
    resultado["Registros_Base"]=resultado["Registros_Base"].fillna(0).astype(int); resultado["Diferença"]=resultado["Valor_Base"]-resultado["Valor_PDF"]
...
def _tabela(df,formatos=None):
    ...
    if "Situação" in df:
        cores={"Conciliado":"background-color:#e8f7ee;color:#166534","Valor divergente":"background-color:#fff7df;color:#92400e","Somente no PDF":"background-color:#feecec;color:#b42318","Somente na Base OB":"background-color:#feecec;color:#b42318"}; estilo=estilo.apply(lambda linha:[cores.get(linha["Situação"],"")]*len(linha),axis=1)
    return estilo
...
def renderizar():
    ...
    with st.container(border=True):
        ...
        with c_comp: executar=st.button("🔎 Comparar pagamentos",type="primary",use_container_width=True)
        with c_voltar: voltar=st.button("← Voltar ao relatório",use_container_width=True)
    ...
    with aba3:
        chaves=set(zip(divergencias["Credor_Chave"],divergencias["GD"])); mostrar=detalhe[detalhe.apply(lambda r:(r["Credor_Chave"],r["GD_Conferencia"]) in chaves,axis=1)].copy()
        if mostrar.empty: st.info("Não há detalhes adicionais para exibir.")
        else:
            mostrar["Valor"]=mostrar["Valor_Conferencia"].map(_brl); st.dataframe(mostrar[["Data Emissão","Credor_Exibicao","GRUPO","Tipo_Item_Conferencia","Fonte","Número","Valor"]].rename(columns={"Data Emissão":"Data","Credor_Exibicao":"Credor","GRUPO":"Grupo","Tipo_Item_Conferencia":"Classificação","Número":"OB"}),use_container_width=True,hide_index=True,height=420)